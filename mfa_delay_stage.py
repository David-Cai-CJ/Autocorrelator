...
from src.XPS_C8_drivers import XPS
from src.base import InstrumentException
from src.utils import is_valid_IP, timeout
...
#         _errcheck(self._driver.KillAll(self.socket_id))
        ...
        errcode, self.min_limit, self.max_limit = _errcheck(
            self._driver.PositionerUserTravelLimitsGet(
                self.socket_id, self.positioner)
        )
    ...
    def relative_move(self, move):
        ...
        _errcheck(
            self._driver.GroupMoveRelative(
                self.socket_id, self.positioner, [move])
        )
    ...
    def absolute_move(self, move):
        ...
        _errcheck(
            self._driver.GroupMoveAbsolute(
                self.socket_id, self.positioner, [move])
        )
        return self._wait_end_of_move()


class MFA_PPD(DelayStage):
    """
    Interface to an MFA-PPD delay-stage connected
